...
const taskReducer = (state=initialState,{type,payload}) => {
    ...
    case EDIT_TASK:
        return{...state, taskList:state.taskList.map((elem)=>elem.id===payload.id? {...elem,description:payload.value}: elem),}       
    case FILTER_TASK:
        return {...state,taskList: state.taskList.map((elem) =>elem.id === payload ? { ...elem, isDone: !elem.isDone } : elem),}    
    case DELETE_TASK:
        return{...state, taskList:state.taskList.filter((elem) => elem.id !== payload),}  
